# Flask application stack for Kubernetes on Wodby

Deploy Flask applications on Kubernetes with Wodby.

This repository defines the Wodby stack manifests and default service
composition for Flask.

<!-- wodby:generated:start -->

## Stack contract

- [Flask stack on Wodby](https://wodby.com/stacks/flask)
...
- [Python (Flask) service](https://github.com/wodby/service-flask)
...
| --- | --- |
| Python<br>`flask` | required; enabled by default; links: `db` → `postgres`, `redis` → `valkey`, `sendmail` → `mailpit` |
...
Start from [Flask boilerplate](https://github.com/wodby/flask-boilerplate), or connect your own compatible source
repository.
